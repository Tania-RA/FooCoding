...
const keys = Object.keys(pokemonData.sprites);
...
keys.forEach((key) => {
  let imageSource = (`${pokemonData.sprites[key]}`);
  console.log(imageSource);

  //imageSource also has null data hence has to have conditions
    if (imageSource == 'null') {
      console.log('is null');
    } else {
      const title = document.createElement("h2");
      title.setAttribute("class", "title");
      title.innerText = key.toUpperCase().replace('_'," ");
      const image = document.createElement('img');
      image.src = (`${pokemonData.sprites[key]}`);
      image.setAttribute("class","img1")
      let br=document.createElement("br");

      divOne.appendChild(image);
      divOne.appendChild(title);
      divOne.appendChild(br);
    }
});
